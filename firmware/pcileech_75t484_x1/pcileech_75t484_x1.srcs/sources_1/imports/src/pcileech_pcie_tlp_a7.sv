//
// Starlight DMA Firmware - TLP Handler (Modified)
// Based on PCILeech FPGA by Ulf Frisk
//
// MODIFICATIONS FOR AMD IOMMU BYPASS:
//   - Shadow CFG space includes ACS extended capability (0x000D)
//   - ACS enables proper IOMMU group assignment on AMD systems
//   - TLP filter properly handles all CfgRd/CfgWr types for shadow CFG
//   - Completions (Cpl/CplD) are always passed through
//
// AMD IOMMU KERNEL PARAMETERS (on the DMA host / second PC):
//   For passthrough mode: amd_iommu=on iommu=pt
//   For full IOMMU:       amd_iommu=on iommu=on
//   The firmware handles both modes through proper config space emulation.
//
